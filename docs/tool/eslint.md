...
module.exports = {
  ...
  // 配置脚本解析，默认为esprima，可选择"babel-eslint"
  parser: 'babel-eslint',
  // Environment预设好的其他环境的全局变量,可选值[brower, node环境变量, es6环境变量, mocha环境]
  evt: {
    browser: true,
    node: true,
    es6: true,
  ...
  /**
   * 自定义规则,http://eslint.org/docs/rules/
   * error级别系数。系数0为不提示(off)、1为警告(warn)、2为错误抛出(error)，可指定范围，如[1,4]
   */
  rules: {
    // 每个对象方法/函数都必要写备注（参数、返回值)   ????
    'valid-jsdoc': ['error等级', { requireReturn: false }]
    // 需要写备注的地方
    'require-jsdoc': ['error等级', {
      require: {
    ...
    // 忽略掉那些方法/函数可以不需要写注解
    'object-literal-jsdoc/obj-doc': ['error等级', {
      ignoreMethods: ['beforeCreate', 'beforeMount']
    }],
